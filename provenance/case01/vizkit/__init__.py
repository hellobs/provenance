# -*- coding: utf-8 -*-
"""可插拔可视化核心:统一事件契约 + 插件注册表。

设计目标
--------
1. 引擎/注入层只负责"产生事件",不关心谁来画;
2. 一个 run 的记录(离线)与 mavis 实时回调(在线)归一化成同一套事件;
3. 任意可视化后端作为插件注册即可接入(小镇风格 Phaser / 审查页 / 平台嵌入 / 控制台);
4. 插件可以选择消费"实时事件流"或"整份 run 记录",两者都走同一契约。

事件契约(与 provenance live 与 mavis protocol 对齐)
--------------------------------------------------
{"type": "init",     "sim_time": str, "agents": [name, ...]}
{"type": "time",     "sim_time": str, "step": int}
{"type": "agent",    "name": str, "coord": [x, y], "action": str, "location": str, "sim_time": str}
{"type": "chat_line","speaker": str, "text": str, "sim_time": str}
{"type": "story",    "id": str, "event_type": str, "content": str, "targets": [...], "sim_time": str}
{"type": "snapshot", "sim_time": str, "agents": {...}}
"""
