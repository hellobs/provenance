...
> 建立日期：2026-09-16
> 前置事实：实现侧的本地 HF 提交 `7ae216f` 已直接在 main 上（未走 PR）；本计划 Phase 0 与之相关但独立。
...
> 变更记录（2026-09-16）：原阶段 0（补齐 实现侧的本地 HF 提交的评审、测试与 CI）**决定不做**。不做项与已知后果见文末。
...
1. 字段级对比：映射层 `record.py` 已实现并通过测试（含与真实 `runs/demo-3/run.json` 顶层键交叉校验）；
   ...
   同日核对旧运行方式记录时发现并修正一处映射偏差：`branch_action.timeline` 原样照抄分支号，
...
> 状态修正(2026-09-17)：下列产物与文档**已就绪**,但"是否替换已冻结的旧运行方式、
> 是否让平台换样本"是留给决策者的拍板项,**不在本阶段判定为已完成**;
> 待人工确认后才推进正式替换与平台通知。

1. 旧运行方式归档：打标签 `archive/case01-old-engine`（`d41cdec`），README 注明
...
**待人工确认的关口**：① 是否正式替换旧运行方式并让平台换样本；② 是否现在做
...
5. 排期冲突：平台侧协作方平台联调配合、实现侧的 HF 后端、演示视频录制均已确认可后置。

---

## 不做项与已知后果（2026-09-16 决定）

原阶段 0（补齐 实现侧 HF 提交的流程）不做，以下后果已知并接受：
