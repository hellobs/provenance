...
> `case01/viz.py` 属已归档旧运行方式）。
> 目标：**一份数据、多种画法**——运行方式/注入层只产生事件，可视化后端按插件接入。
...
这些不该各自去改运行方式。因此把"产生事件"与"怎么画"分开：前者在运行方式/注入层，后者全部是插件。
...
- `mavis_vizkit.Fanout`：把事件广播给多个插件；**单个插件抛错会被隔离**，不影响运行方式与其它插件。
...
- uvicorn 跑在守护线程；运行方式线程每产生一个事件就调 `on_event()`，
