...
def default_scenario_dir() -> str:
    """场景数据根:裸包无数据返回空;case01 侧返回 case01/injector/scenario。"""
    from mavis_case01_injector import _providers
    return _providers.default_scenario()


def __getattr__(name):
    # 兼容旧引用:DEFAULT_SCENARIO 从常量变为惰性解析(阶段 3 抽包)
...
def rerun_router_only(path: str, router_llm=None, external_router: bool = False,
                      write: bool = False, reflection_mod=None) -> dict:
